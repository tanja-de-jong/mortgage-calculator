...
public class Berekenaar {
    ...
    public Berekenaar(double startBedrag) throws IOException, ParseException {
...
*/
       ...
       /* verstrekkers.add(rabobankAnnuitair);
        ...
        String content = FileUtils.readFileToString(new File("verstrekkers.json"), "UTF-8");
        JSONArray verstrekkersJson = new JSONArray(content);
        ...
        for (int i=0; i<verstrekkersJson.length(); i++) {
            ...
            boolean annuitair = verstrekkerJson.getBoolean("annuitair");
            Map<String,String> rentesJson = new ObjectMapper().readValue(verstrekkerJson.getJSONObject("rentes").toString(), HashMap.class);
            Map<Double,Double> rentes = new HashMap();
            for (String key : rentesJson.keySet()) {
                rentes.put(Double.parseDouble(key), Double.parseDouble(rentesJson.get(key)));
            }

            if (annuitair) {
                Verstrekker verstrekker = new AnnuitaireVerstrekker(naam, rentes);
